                ...
                <div class="col-12 col-sm-8 col-md-6 col-lg-5 col-xl-4">
                    <div class="bg-light rounded p-4 p-sm-5 my-4 mx-3 shadow">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <h3>Registrasi akun disini!</h3>
                        </div>
                        <form action="{{ route('user.register.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control @error('peserta_name') is-invalid @enderror"
                                    id="peserta_name" name="peserta_name" placeholder="Masukkan Nama">
                                <label for="peserta_name">Nama</label>
                                @error('peserta_name')
                            ...
                            <div class="form-floating mb-3">
                                <input type="email" class="form-control @error('peserta_email') is-invalid @enderror"
                                    id="peserta_email" name="peserta_email" placeholder="nama@example.com">
                                <label for="peserta_email">Alamat Email</label>
                                @error('peserta_email')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="form-floating mb-4">
                                <input type="password" class="form-control @error('peserta_password') is-invalid @enderror"
                                    id="peserta_password" name="peserta_password" placeholder="Password">
                                <label for="peserta_password">Password</label>
                                @error('peserta_password')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                @enderror
                            ...
                            <div class="form-floating mb-4">
                                <input type="password" class="form-control" id="peserta_password_confirmation"
                                    name="peserta_password_confirmation" placeholder="Konfirmasi Password" required>
                                <label for="peserta_password_confirmation">Konfirmasi Password</label>
                            </div>
                            <button type="submit" class="btn btn-primary py-3 w-100 mb-4">Daftar</button>
                            <p class="text-center mb-0">Already have an Account? <a href="{{ route('user.login') }}">Log
                                    In</a></p>
                        </form>
                    </div>
                </div>
